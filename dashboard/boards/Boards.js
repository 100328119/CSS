                        ...
                        <FormControl className="flex w-full sm:w-320 mx-16" variant="outlined">
                            ...
                            >
                                ...
                                {departments.map(department => (
                                    <MenuItem value={department.value} key={department._id}>{department.label}</MenuItem>
                                ))}
                            </Select>
                        </FormControl>
                    </div>
                            ...
                            return (
                                <div className=" pb-24 sm:w-1/8 lg:w-1/4 sm:p-8" key={board._id}>
                                    <Card elevation={1} className="flex flex-col h-256 w-256">
                                        ...
                                        >
                                            <Typography className="font-medium truncate" color="inherit">{department.label}</Typography>
                                            <div className="flex items-center justify-center opacity-75">
                                            </div>

                                        </div>
                                        <Link
                                            to={`/dashboard/boards/${board._id}/${board.uri}`}
                                            className={classNames(classes.board, "flex flex-col items-center justify-center w-full h-full rounded py-24")}
                                            role="button"

                                        >
                                        <Typography className="text-center text-16 font-400">{board.name}</Typography>
                                        <Typography className="text-center text-13 font-600 mt-4" color="textSecondary">{board.updated}</Typography>
                                        </Link>
